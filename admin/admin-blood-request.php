...
$id=$_POST['hidd'];
$status="Rejected";
$q=$db->prepare("UPDATE `blood_request` SET `status`=:status WHERE sno=:id");
$q->bindValue('id',$id);
...
else if(isset($_POST['approve']))
{
    $id=$_POST['hidd'];
        ...
        $q=$db->prepare("SELECT * FROM blood_request WHERE sno=:id");
        $q->bindValue('id',$id);
        $q->execute();
        ...
        $q=$db->prepare("SELECT * FROM blood_stock WHERE bgroup=:bgroup");
        $q->bindValue('bgroup',$bgroup);
        $q->execute();
        $rl=$q->fetch(PDO::FETCH_OBJ);
        $stock_available=$rl->stock;
        // echo "<script>alert('$stock_available');</script>";
        // echo "<script>alert('$unit');</script>";
        if($stock_available >= $unit)
        {
            ...
            // echo "<script>alert('Stock Does Not Have Enough Blood To Approve This Request, Only $stock_available Unit Available');</script>";
            $flag="ok";
        }
  ...
  <div class="side-bar">
    ...
    <div class="d-flex flex-column flex-shrink-0 p-3 text-white bg-dark bar" style="position: fixed; margin-top: 50px;">
      ...
      <ul class="nav nav-pills flex-column mb-auto">
        ...
        <li>
          <a href="admin-blood-request.php" class="nav-link active">
    ...
    <div class="main">
      <div class="cont-card">
        ...
        if($flag == "ok"){
            ...
            margin-bottom: 17px; '>
            <strong>Stock Does Not Have Enough Blood To Approve This Request, Only $stock_available Unit Available</strong>
            </div>";
        }
        else if($flag=="not_ok")
        {
     ...
     ?>
        <table class="table table-bordered"
          ...
          <?php
              $q=$db->query("SELECT * FROM blood_request");
            ...
            <tr>
              ...
              <td>
                ...
                <form action="" method="post" style="display:inline;">
                  ...
                  <input type="submit" class="btn btn--radius-2 btn-primary" style="    margin: 6px; width:112px;"
                    value="APPROVE" name="approve">
                </form>
                <form action="" method="post" style="display:inline;">
                  <input type="hidden" name="hidd" value="<?= $rl->sno; ?>">
                  <input type="submit" class="btn btn--radius-2 btn-danger" style="width:112px;" value="REJECT"
                    name="reject">
                </form>
                <?php
                }
                ...
                '><h6>0 Unit Deducted From Stock</h6></div>";
                }
                else if(($rl->status)=="Approved"){
                    echo "<div style='color: white; background-color: #24c724;
                '><h6 style='margin:0;'>$rl->unit Unit Deducted From Stock</h6></div>";
                }
                 ?>
              </td>
