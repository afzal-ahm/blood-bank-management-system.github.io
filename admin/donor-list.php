...
{
  $id=$_POST['hidd'];
  $q=$db->query("SELECT uname FROM donor_registration WHERE sno=$id");
  $rl=$q->fetch(PDO::FETCH_OBJ);
  $del_uname = $rl->uname;
$q=$db->prepare("DELETE FROM donor_registration WHERE sno=:id");
$q->bindValue('id',$id);
$q->execute();
// echo "<script>alert('$del_uname');</script>";

// $del_uname = "john";
$q=$db->prepare("DELETE FROM donor WHERE uname=:del_uname");
$q->bindValue('del_uname',$del_uname);
$q->execute();
// $rl=$q->fetch(PDO::FETCH_ASSOC);
// echo "<pre>";
// print_r($rl);
}
if(isset($_POST['edit']))
